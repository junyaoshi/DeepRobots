...
from Robots.ContinuousDeepRobots_body import ThreeLinkRobot
import datetime
import random
import copy
import numpy as np
...
class DQNAgent:
    INPUT_DIM = 4
    OUTPUT_DIM = 1
    def __init__(self, actions_params=(-pi/16, pi/16, pi/128), memory_size=1500, gamma=0.95, epsilon=1.0,
        ...
        if epsilon_greedy:
            if np.random.rand() <= self.epsilon:
                ...
                while True:
                    ...
                    a1, a2 = temp_robot.move(chosen_action)
    ...
    def act(self, robot, action):
        ...
        reward = 100 * sqrt(((new_x-old_x)**2) + ((new_y-old_y)**2))
        print('reward: ', reward)
        if reward == 0:
            print('incur penalty')
            reward = -10
...
fig.savefig('images/Wheeled body Trial' + TRIAL_NAME + 'Loss vs Number of Iterations '
...
def make_graphs(xs, ys, a1s, a2s, steps, number, trial_name):
    ...
    # fig2.tight_layout()
    fig1.tight_layout()
    fig1.savefig('images/Wheeled body Trial' + trial_name + 'Policy Rollout ' + str(number) + ' '
                 + str(datetime.datetime.now()).replace(' ', '').replace(':','') + '.png')
    plt.close(fig1)
...
for j in range(1):
    ...
    generate_csv(robot_params, "Trial " + str(j+1) + " th rollout "
