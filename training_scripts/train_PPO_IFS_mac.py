...
from custom_eval_class import CustomCallback
from experience_logger import ExpLogger

raw_env = IdealFluidSwimmerEnv(reset_theta = False)
# Optional: PPO2 requires a vectorized environment to run
# the env is now wrapped automatically when passing it to the constructor
vec_env = DummyVecEnv([lambda: raw_env])

test_raw_env = IdealFluidSwimmerEnv(reset_theta = False)
test_vec_env = DummyVecEnv([lambda: test_raw_env])

trial_name = "trial_15_17"
...
#model.learn(total_timesteps=params['total_timesteps'], tb_log_name="trial_run", callback = CustomCallback(vec_test_env=test_vec_env, trial_name = trial_name))
model.learn(total_timesteps=params['total_timesteps'], tb_log_name="trial_run", callback = None)

env = vec_env.envs[0]
#obs_prev = env.reset_w_theta(pi)
env.enable_test_mode()

for j in range(1):
    results_dir = os.path.join("results", "LearningResults", "PPO_IdealFluidSwimmer", trial_name, "theta_int_" + str(j))
...
with open(str(results_dir) + '/PPO_data_' + trial_name + '.txt', 'w') as file:
    file.write(json.dumps(data_dict))
