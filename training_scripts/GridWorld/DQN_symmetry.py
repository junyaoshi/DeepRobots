...
class DQNAgent():
    ...
    def replay_mem(self, batch_size, current_iteration):
        ...
        for state_with_action, reward, next_state, is_done in minibatch:
            ...
            symmetries = self.find_symmetries(state_with_action)
    ...
    def find_symmetries(self, target_state_with_action):
        occurences_counts = {}
        ...
        for i in range(self.reward_trail_length + 1): # + 1 because of the root
            ...
            for node in nodes:
                ...
                if target_state_with_action in node.occurrences:
                    target_occurrence_count = node.occurrences[target_state_with_action]
                ...
                for state_action_key, occurence_count in node.occurrences.items():
                    ...
                    if target_occurrence_count != 0 and state_action_key != target_state_with_action:
        ...
        for state_action_key, intersection_count in occurences_counts_intersection.items():
            similarity = intersection_count / math.sqrt(occurences_counts[state_action_key] * occurences_counts[target_state_with_action])
            if similarity < self.reward_trail_symmetry_threshold:
                continue
            symmetries.append(state_action_key)
        return symmetries