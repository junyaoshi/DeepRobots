...
#a_max_arr = np.linspace(pi, pi + 0.1, NUM_BATCHES, endpoint = True)

a_max_arr = [2.8, 2.8, 2.8, 2.8, 2.8, 2.8, 2.8]

big_trial = "trial_12_60"

for batch in range(NUM_BATCHES):

    a_upper = a_max_arr[batch]
    a_lower = -1*a_upper

    raw_env = IdealFluidSwimmerEnv(a_upper = a_upper, a_lower = a_lower, reset_theta = False)
    print(raw_env.snake_robot.a_upper)
    print(raw_env.snake_robot.a_lower)
    ...
    trial_name = "trial_" + str(batch)
    results_dir = os.path.join("results", "LearningResults", "SAC_IdealFluidSwimmer", big_trial + "_batches", trial_name)
    ...
    model = SAC(MlpPolicy, vec_env, learning_rate=params['learning_rate'], gamma = 0.99, verbose=1, tensorboard_log=tensorboard_dir, ent_coef = 'auto_3.0')
    model.learn(total_timesteps=params['total_timesteps'], tb_log_name="trial_run", reset_theta = False, reset_freq = 500, callback = StateLogger(trial_name = big_trial + "_" + trial_name)) 

    model_dir = os.path.join(results_dir, "sac_model")
