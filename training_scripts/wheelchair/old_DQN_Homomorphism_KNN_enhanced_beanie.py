...
class RewardsModel(nn.Module):
    ...
    def forward(self, states, actions):
        states_for_all_actions = states.unsqueeze(1).repeat(1, len(actions), 1)
        actions_for_all_states = actions.unsqueeze(0).repeat(states.shape[0], 1, 1)
        za = torch.cat([states_for_all_actions, actions_for_all_states], dim=-1)
...
class Loss(nn.Module):
    ...
    def forward(self, abstract_states, transitioned_abstract_states, abstract_next_states, action_embeddings):
        # Transition loss
...
class DQNAgent():
    ...
    def get_abstract_reward_fixation(self, reward):
        reward = self.get_abstract_reward(reward)
        ...
        return self.reward_fixation_in_abstraction[reward]
    ...
    def replay_abstract_model(self):
        ...
        with torch.no_grad():
            ...
            result_list = []
            for i in range(abstract_rewards_tensor.shape[0]):
                for j in range(abstract_rewards_tensor.shape[1]):
                    value = abstract_rewards_tensor[i, j, 0]
                    result_list.append(self.get_abstract_reward_fixation(float(value)))
            abstract_rewards_tensor = torch.stack(result_list).view(abstract_rewards_tensor.shape[0], abstract_rewards_tensor.shape[1], -1)
        reward_fixation_loss = square_dist(abstract_states_tensor, abstract_rewards_tensor.mean(dim=1)).mean()
        trans_loss, symmetry_loss = self.loss_function(abstract_states_tensor,
    ...
    def replay_mem(self, batch_size):
        ...
        if self.params['K_for_KNN'] < math.floor(len(self.abstract_state_holders) / self.params['abstract_KNN_interval']) and self.episode_counter >= self.params['equivalent_exploitation_beginning_episode']:
            ...
            for index, (state, action, reward, symmetries) in enumerate(zip(states, actions, rewards, symmetrie_all)):
                ...
                if len(symmetric_states) == 0:
                    loss = loss + F.mse_loss(outputs_selected[index], targets[index])
                    continue
