...
class StateLogger(BaseCallback):
    """
    ...
    def __init__(self, verbose=0, reset_freq = 100, vec_test_env=None, trial_name=None):
        super(StateLogger, self).__init__(verbose)
        ...
        self.file.write('x,y,theta,ahead,atail,a1dot,a2dot\n')
    ...
    def _on_step(self) -> bool:
        ...
        ahead = state[1]
        atail = state[2]
        a1dot = model_robot.a1dot
        a2dot = model_robot.a2dot

        data_str = str(x) + ',' + str(y) + ',' + str(theta) + ',' + str(ahead) + ',' + str(atail) + ',' + str(a1dot) + ',' + str(a2dot) + '\n'
