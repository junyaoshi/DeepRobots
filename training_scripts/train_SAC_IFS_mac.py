...
raw_env = IdealFluidSwimmerEnv(reset_theta = False, a_upper=7/8*pi, a_lower=-7/8*pi)
...
model = SAC(MlpPolicy, vec_env, learning_rate=params['learning_rate'], verbose=1, tensorboard_log=tensorboard_dir)#, ent_coef = 'auto_3.0')
model.learn(total_timesteps=params['total_timesteps'], tb_log_name="trial_run", reset_theta = False, reset_freq = 500, callback = None) 

model_dir = os.path.join("results", "LearningResults", "SAC_IdealFluidSwimmer", trial_name, "sac_model_" + trial_name)
